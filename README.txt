...
allowing interventions on pipes.  The server side program `tiis' opens
a slave process and duplicates its IO to the terminal inside of which
`tiis' is running.  At the same time, `tiis' communicates with the
client side program `tiic' (or another network program such as telnet
or nc), via either network sockets or pipes, and injects what it
receives from the client to the slave process as if the input were
...
`tiis' resides.  The client program `tiic' runs in another terminal
(could be on another machine).  From a user's perspective, interacting
with `tiic', or directly typing in the terminal `tiis' resides, are
the same as directly interacting with the slave process.

    The whole point of creating such a program is that `tiic' can be
run as a slave (i.e., via popen) to control the desired program (i.e.
gnuplot, in such case gnuplot will be the slave process running under
